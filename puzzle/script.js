class Box {
  ...
  class Game {
    ...
    render() {
      ...
      if (status === "ready") newButton.textContent = "Shuffle and start";
      if (status === "playing") newButton.textContent = "Reset";
      if (status === "won") newButton.textContent = "Shuffle and start";
      newButton.addEventListener("click", () => {
        clearInterval(this.tickId);
        this.tickId = setInterval(this.tick, 1000);
        this.setState(State.start());
      });
      document.querySelector(".head button").replaceWith(newButton);
